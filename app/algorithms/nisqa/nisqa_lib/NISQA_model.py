...
class NisqaModel(object):
    ...
    def _train_mos(self):
        ...
        for epoch in range(self.args['tr_epochs']):
            ...
            if self.args['tr_verbose'] > 0:
                print('<---- Validation ---->')
            NL.predict_mos(self.model, self.ds_val, self.args['tr_bs_val'], self.dev,
                           num_workers=self.args['tr_num_workers'])
    ...
    def _train_dim(self):
        ...
        for epoch in range(self.args['tr_epochs']):
            ...
            NL.predict_dim(self.model, self.ds_val, self.args['tr_bs_val'], self.dev,
                           num_workers=self.args['tr_num_workers'])
    ...
    def _loadDatasets(self):
        ...
        elif self.args['mode'] == 'predict_list':
            self._loadDatasetList()
    ...
    def _loadDatasetList(self):
        # 输入的数据是一个列表
        # 列表数据通过deg_list传入的

        files = self.args['deg_list']
        files = [os.path.basename(file) for file in files]
        df_val = pd.DataFrame(files, columns=['deg'])
        data_dir = os.path.dirname(self.args['deg_list'][0])
        print('# files: {}'.format(len(df_val)))
        if len(df_val) == 0:
            raise ValueError('No wav files found in data_dir')
    ...
    def _loadDatasetsCSV(self):
        ...
        if not set(self.args['csv_db_train'] + self.args['csv_db_val']).issubset(dfile.db.unique().tolist()):
            missing_datasets = set(self.args['csv_db_train'] + self.args['csv_db_val']).difference(
    ...
    def _loadModel(self):
        ...
        # 三种模式 NISQA 只有mos  NISQA_DIM是多维度的  NISQA_DE是双端音频的
    ...
    def _getDevice(self):
        """
        Train on GPU if available. 判断是不是支持cuda以及是否使用平行训练
        """
        ...
        if "tr_parallel" in self.args:
            if (self.dev == torch.device("cpu")) and self.args['tr_parallel'] is True:
