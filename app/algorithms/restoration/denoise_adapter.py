...
class DenoiseRestorerAdapter(BaseRestorer):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if DenoiseRestorerAdapter._DenoiserRegistry is None and not DenoiseRestorerAdapter._import_attempted:
                ...
                logger.info(f"[DenoiseAdapter] 项目根目录: {project_root}")
                logger.info(f"[DenoiseAdapter] 当前 sys.path 前5项: {sys.path[:5]}")

                # 确保项目根目录在路径最前面
                if project_root in sys.path:
                    sys.path.remove(project_root)
                sys.path.insert(0, project_root)
                logger.info(f"[DenoiseAdapter] 已添加项目根目录到 sys.path[0]")

                # 使用绝对导入
                logger.info(f"[DenoiseAdapter] 尝试导入 DenoiserRegistry...")
                ...
                try:
                    ...
                    logger.info(f"[DenoiseAdapter] 成功导入 DenoiserRegistry (方法1)")
                except ImportError as e1:
                    logger.info(f"[DenoiseAdapter] 方法1导入失败: {e1}")
                    # 方法2：使用 importlib 动态导入
                    denoise_path = os.path.join(project_root, "app", "algorithms", "denoise", "__init__.py")
                    logger.info(f"[DenoiseAdapter] 尝试从 {denoise_path} 导入")
                    ...
                    if os.path.exists(denoise_path):
                        try:
                            ...
                            logger.info(f"[DenoiseAdapter] 成功导入 DenoiserRegistry (方法2)")
                        except Exception as e2:
                            logger.info(f"[DenoiseAdapter] 方法2导入也失败: {e2}")
                            import traceback
                            traceback.print_exc()
                            DenoiseRestorerAdapter._import_attempted = True
                            return False
                    else:
                        logger.info(f"[DenoiseAdapter] 找不到 denoise 模块: {denoise_path}")
                        DenoiseRestorerAdapter._import_attempted = True
                        return False
            ...
            if DenoiseRestorerAdapter._DenoiserRegistry is None:
                logger.warning("[DenoiseAdapter] DenoiserRegistry 导入失败")
                return False
            ...
            if self._denoiser is None:
                logger.warning(f"降噪算法 {self._denoiser_name} 不可用")
                return False

            if not self._denoiser.is_initialized():
                success = self._denoiser.initialize()
                if not success:
                    logger.error(f"降噪算法 {self._denoiser_name} 初始化失败")
                    return False
            ...
            logger.warning(f"降噪模块不可用: {e}")
            return False
        except Exception as e:
            logger.error(f"适配器初始化失败 ({self._denoiser_name}): {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def _find_project_root(self) -> str:
        ...
        logger.info(f"[DenoiseAdapter] 当前文件目录: {current}")
        ...
        for _ in range(10):
            ...
            if os.path.isdir(os.path.join(parent, "app")) and os.path.isdir(os.path.join(parent, "backend")):
                logger.info(f"[DenoiseAdapter] 找到项目根目录: {parent}")
                return parent
            ...
            for marker in ["models/clearvoice", ".git"]:
                ...
                if os.path.exists(marker_path):
                    logger.info(f"[DenoiseAdapter] 通过标志 {marker} 找到项目根目录: {parent}")
                    return parent
        ...
        # 如果找不到，使用当前工作目录
        cwd = os.getcwd()
        logger.info(f"[DenoiseAdapter] 未找到项目根目录，使用当前工作目录: {cwd}")
        return cwd
