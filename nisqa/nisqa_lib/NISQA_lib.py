...
class Framewise(nn.Module):
    ...
    def __init__(
        ...
        if cnn_model == 'adapt':
            ...
            raise NotImplementedError('Framewise model not available')
...
class DFF(nn.Module):
    """
    DFF: Deep Feed-Forward network that was used as baseline framewise model as
    comparison to the CNN.
    """
...
class SelfAttentionLayer(nn.Module):
    """
    SelfAttentionLayer: The SelfAttentionLayer that is used by the SelfAttention module.
    """
    ...
    def forward(self, src, n_wins=None):
        ...
        if n_wins is not None:
            mask = ~((torch.arange(src.shape[0])[None, :]).to(src.device) < n_wins[:, None].to(torch.long).to(
                src.device))
        else:
            mask = None
...
# %% Spectrograms
def segment_specs(file_path, x, seg_length, seg_hop=1, max_length=1400):
    '''
