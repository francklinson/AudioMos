...
from ASTAttentionPooling import AttentionPooling
...
@dataclass
class ASTAttentionPoolintConfig(BaseConfig):
    def __init__(self):
        super().__init__()
        self.model_path = 'AST_Results/finetuned_ast_sota_model.pt'
        self.labelmap_path = 'AST_Results/finetuned_ast_sota_label_map.pkl'
        ...
        self.output_dir = 'FINAL_ANALYSIS_RESULTS_AST_WithAttention'
        self.sr = 22050
...
class ASTForSongIDWithAP(nn.Module):
...
class ASTAttentionPoolingEvaluator(BaseEvaluator):
    """Handles model evaluation and analysis"""

    def __init__(self, config: ASTAttentionPoolintConfig):
...
def main():
    config = ASTAttentionPoolintConfig()
    evaluator = ASTAttentionPoolingEvaluator(config)
